...
Демонстрация проекта: https://rutube.ru/video/private/97086fb20d2f69bca102303a0924663b/?p=z0bUbEiLBQlb_Aklwwve8A

Краткое описание: система включает три уровня доступа: ученик, повар и администратор. Для ученика реализованы функции регистрации и авторизации, просмотра меню завтраков и обедов, разовой оплаты или покупки абонемента, отметки о получении питания, указания пищевых аллергий и предпочтений, а также оставления отзывов о блюдах. Для повара предусмотрены учет выданных завтраков и обедов, контроль остатков продуктов и готовых блюд, оформление заявок на закупку, а также изменение перечня аллергенов. Администратор получает доступ к формированию отчетов по питанию и затратам, согласованию заявок на закупку, статистике оплат и посещаемости и изменению перечня аллергенов. Дополнительно система включает модуль уведомлений для учеников и сотрудников.
...
```
Важно:
- Аргументы нужно передавать через `--`. Иначе Vite воспримет их как `root`, и на `http://localhost:5173/` будет `HTTP 404`.
- Если вдруг сервер слушает только IPv6 (`::1`), открывай `http://[::1]:5173/` или запускай с `--host 127.0.0.1`.
